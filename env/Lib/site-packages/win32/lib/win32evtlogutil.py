...
def AddSourceToRegistry(
    ...
    """Add a source of messages to the events log.
    ...
    full message text appears in the events log.
    ...
    you do not get language translation, and the full text is stored in the events log,
    blowing the size of the log up.
    """

    # When an application uses the RegisterEventSource or OpenEventLog
    # function to get a handle of an events log, the events logging service
...
def RemoveSourceFromRegistry(appName, eventLogType="Application"):
    """Removes a source of messages from the events log."""
...
def ReportEvent(
    ...
    """Report an events for a previously added events source."""
    # Get a handle to the Application events log
    hAppLog = win32evtlog.RegisterEventSource(None, appName)

    # Now report the events, which will add this events to the events log */
    win32evtlog.ReportEvent(
        hAppLog,  # events-log handle \
        eventType,
        eventCategory,
        eventID,
        sid,
...
def FormatMessage(eventLogRecord, logType="Application"):
    """Given a tuple from ReadEventLog, and optionally where the events
    ...
    # From the events log source name, we know the name of the registry
