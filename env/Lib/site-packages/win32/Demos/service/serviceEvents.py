...
# Note that all output is written as events log entries - so you must install
# and start the service, then look at the events log for messages as events
# are generated.
...
# sleep and waking it, inserting a memory stick, etc then check the events log

# Most events notification support lives around win32gui
...
class EventDemoService(win32serviceutil.ServiceFramework):
    ...
    def SvcOtherEx(self, control, event_type, data):
        ...
        if control == win32service.SERVICE_CONTROL_DEVICEEVENT:
            ...
            msg = f"A device events occurred: {event_type:x} - {info}"
            ...
            msg = "A power events: setting %s" % data
            ...
            msg = f"Session events: type={event_type}, data={data}"
        else:
            msg = "Other events: code=%d, type=%s, data=%s" % (control, event_type, data)
