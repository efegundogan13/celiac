...
# Keepalive connections are put back in the loop waiting for an events.
# If no events happen after the keep alive timeout, the connection is
# closed.
...
class ThreadWorker(base.Worker):
    ...
    def run(self):
        # init listeners, add them to the events loop
        ...
        try:
            ...
            if keepalive and self.alive:
                ...
                with self._lock:
                    ...
                    # add the socket to the events loop
                    self.poller.register(conn.sock, selectors.EVENT_READ,
                                         partial(self.on_client_socket_readable, conn))
            else:
                self.nr_conns -= 1
