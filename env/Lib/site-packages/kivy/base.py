...
access directly the events loop of an application.
'''
...
class EventLoopBase(EventDispatcher):
    '''Main events loop. This loop handles the updating of input and
    dispatching events.
    '''
    ...
    def set_window(self, window):
        '''Set the window used for the events loop.
        '''
        self.window = window
    ...
    def add_event_listener(self, listener):
        '''Add a new events listener for getting touch events.
        '''
        if listener not in self.event_listeners:
            self.event_listeners.append(listener)

    def remove_event_listener(self, listener):
        '''Remove an events listener from the list.
        '''
    ...
    def start(self):
        ...
        .. versionchanged:: 2.1.0
            Method can be called multiple times, but events loop will start only
            once.
        '''
    ...
    def stop(self):
        ...
        .. versionchanged:: 2.1.0
            Method can be called multiple times, but events loop will stop only
            once.
        '''
        if self.status != 'started':
            return
    ...
    def post_dispatch_input(self, etype, me):
        ...
        when we want to dispatch an input events. The events is dispatched to
        ...
        if not me.is_touch:
            # Non-touch events must be handled by the events manager
            return
        me.grab_state = True
        for weak_widget in me.grab_list[:]:
            ...
            if etype == 'begin':
                ...
                # a down events are nearly uniq here.
    ...
    def _dispatch_input(self, *ev):
        # remove the save events for the touch if exist
    ...
    def idle(self):
        ...
        if len(self.event_listeners) == 0:
            Logger.error('Base: No events listeners have been created')
...
def _runTouchApp_prepare(widget=None):
    ...
    #    a redisplay events.
    #
    # 2. if no window is created, we are dispatching events loop
...
def runTouchApp(widget=None, embedded=False):
    ...
    :Parameters:
        ...
        `embedded`
            No events dispatching is done. This will be your job.

        `widget + embedded`
            No events dispatching is done. This will be your job but
...
async def async_runTouchApp(widget=None, embedded=False, async_lib=None):
    ...
    that can be run in an existing async events loop.
