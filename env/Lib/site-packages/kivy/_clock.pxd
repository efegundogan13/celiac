...
cdef class ClockEvent(object):
    ...
    '''The :class:`CyClockBase` instance associated with the events.
    '''
    cdef public int loop
    '''Whether this events repeats at intervals of :attr:`timeout`.
    '''
    ...
    """A Optional callback for this events, which if provided is called by the clock
    when the clock is stopped and the events was not ticked.
    """
    cdef public object weak_clock_ended_callback

    cdef public int release_ref
    """If True, the events should never release the reference to the callbacks.
...
cdef class FreeClockEvent(ClockEvent):
    ...
    '''Whether this events was scheduled as a free events.
    '''
...
cdef class CyClockBase(object):
    ...
    '''If the remaining time until the events timeout is less than :attr:`clock_resolution`,
    ...
    '''The first events in the chain. Can be None.
    '''
    cdef public ClockEvent _next_event
    '''During frame processing when we service the events, this points to the next
    events that will be processed. After ticking an events, we continue with
    :attr:`_next_event`.

    If a events that is canceled is the :attr:`_next_event`, :attr:`_next_event`
    is shifted to point to the after after this, or None if it's at the end of the
    chain.
    '''
    cdef public ClockEvent _cap_event
    '''The cap events is the last events in the chain for each frame.
    For a particular frame, events may be added dynamically after this events,
    ...
    '''The last events in the chain. New events are added after this. Can be None.
    '''
