...
:class:`ClockEvent` returned by it. This ensures that you can call the events
...
with :meth:`CyClockBase.schedule_once`::
    ...
    events = Clock.create_trigger(my_callback)
    events()
    events()

    # will also run the callback only once before the next frame
    events = Clock.schedule_once(my_callback)  # now it's already scheduled
    events()  # won't be scheduled again
    events()

In addition, it is more convenient to create and bind to
the triggered events than using :meth:`CyClockBase.schedule_once` in a
function::
...
An events scheduled with :meth:`CyClockBase.schedule_once`,
...
multiple ways. E.g::
    ...
    # call my_callback every 0.5 seconds
    events = Clock.schedule_interval(my_callback, 0.5)
...
discouraged because it's significantly slower than when using the events.
...
callback in kivy's thread and then process some result. If the events is not
...
called when the events is processed. But, if the clock is stopped before it can
be processed, if the application exited normally (and the app was started) and
the events wasn't canceled, and the callbacks are not garbage collected, then
...
raised when the events was scheduled, then one of these callbacks will be
called - either ``callback`` if the events executed normally, or
``clock_ended_callback`` if the clock is stopped while the events is scheduled.
...
If an events's callback raises an exception, before the exception handler is
...
.. versionchanged:: 2.0.0
    Prior to Kivy 2.0.0, an exception raised in a events's callback would
    ...
    From 2.0.0 onward, if a events's exception is handled by an
...
of all the Clock scheduling methods. By free, we mean the events is free from
the fps because it's not fps limited. E.g.
:meth:`CyClockBaseFree.create_trigger_free` corresponds to
:meth:`CyClockBase.create_trigger`. Only when an events scheduled using the
``Clock.xxx_free`` methods is present will the clock interrupt and execute
the events during the interval. So, if no ``free`` events is present the clock
...
seconds, respectively when it was a free events and 0.02403s, 0.02405s, and
0.04829s, respectively when it wasn't.

Free Only Clock
^^^^^^^^^^^^^^^^^

The Free clock executes all events when a free events was scheduled. This
results in normal events also being execute in the middle of the interval
when a free events is scheduled. For example, above, when a free events was
absent, a normal events with a 0.001s timeout was delayed for 0.02405s. However,
if a free events happened to be also scheduled, the normal events was only
...
default clock. For example, in the presence of a free events, a normal events
...
class ClockBaseBehavior(object):
    ...
    def init_async_lib(self, lib):
        ...
        This can be called anytime before the kivy events loop has started,
    ...
    def post_idle(self, ts, current):
        ...
        # compute how long the events processing takes
...
class ClockBaseInterruptBehavior(ClockBaseBehavior):
    ...
    def idle(self):
        ...
        # will cause a timeout of the events on the next idle due to on_schedule
