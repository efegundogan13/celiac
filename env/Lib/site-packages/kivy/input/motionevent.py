...
3. :class:`~kivy.base.EventLoopBase` dispatches all motion events using
   `on_motion` events to all registered listeners including the
   :class:`~kivy.core.window.WindowBase`.
4. Once received in :meth:`~kivy.core.window.WindowBase.on_motion` events
   (touch or non-touch) are all registered managers. If a touch events is not
...
Motion events and events managers
--------------------------------

A motion events is a touch events if its :attr:`MotionEvent.is_touch` is set to
`True`. Beside `is_touch` attribute, :attr:`MotionEvent.type_id` can be used to
check for events's general type. Currently two types are dispatched by
input providers: "touch" and "hover".

Event managers can be used to dispatch any motion events throughout the widget
tree and a manager uses `type_id` to specify which events types it want to
...
when dispatching a specific events::
    ...
    class MouseTouchManager(EventManagerBase):
        ...
        def dispatch(self, etype, me):
            ...
            if me.device == 'mouse':
                ...
                # Dispatch mouse touch events to widgets which registered
...
class MotionEvent(MotionEventBase):
    '''Abstract class that represents an input events.
    ...
    def __init__(self, device, id, args, is_touch=False, type_id=None):
        ...
        #: (Experimental) String to identify events type.
        #:
        #: .. versionadded:: 2.1.0
        self.type_id = type_id

        #: (Experimental) Used by a events manager or a widget to assign
        ...
        #: Uniq ID of the events. You can safely use this property, it will be
        #: never the same across all existing events.
        self.uid = MotionEvent.__uniq_id

        #: Device used for creating this events.
        self.device = device
        ...
        #: Used to determine which widget the events is being dispatched to.
        ...
        #: Profiles currently used in the events.
        self.profile = []

        #: Id of the events, not unique. This is generally the Id set by the
        ...
        #: Shape of the touch events, subclass of
        ...
        #: Initial time of the events creation.
        self.time_start = time()

        #: Time of the last update.
        self.time_update = self.time_start

        #: Time of the end events (last events usage).
        self.time_end = -1

        #: Indicate if the touch events is a double tap or not.
        self.is_double_tap = False

        #: Indicate if the touch events is a triple tap or not.
        #:
        #: .. versionadded:: 1.7.0
        ...
        #:
        #: This attribute is needed because events provider can make many calls
        #: to :meth:`MotionEvent.move`, but for all those calls events is
        ...
        #: Flag that first dispatch of this events is done.
    ...
    def grab(self, class_instance, exclusive=False):
        '''Grab this motion events.

        If this events is a touch you can grab it if you want to receive
    ...
    def ungrab(self, class_instance):
        '''Ungrab a previously grabbed motion events.
        '''
        class_instance = weakref.ref(class_instance.__self__)
    ...
    def copy_to(self, to):
        '''Copy some attribute to another motion events object.'''
    ...
    @property
    def opos(self):
        '''Return the initial position of the motion events in the screen
        coordinate system (self.ox, self.oy).'''
        return self.ox, self.oy

    @property
    def ppos(self):
        '''Return the previous position of the motion events in the screen
    ...
    def is_mouse_scrolling(self, *args):
        '''Returns True if the touch events is a mousewheel scrolling
