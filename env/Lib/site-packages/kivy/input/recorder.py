...
Once activated, the recorder will listen for any input events and save its
...
dispatch it to the events loop.
...
-----------------
  ...
  - Can replay only complete records. If a begin/update/end events is missing,
...
class Recorder(EventDispatcher):
    ...
    '''Attributes to record from the motion events.
    ...
    '''Profile to save in the fake motion events when replayed.
