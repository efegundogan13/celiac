...
single touch can generate one events from the mouse provider and another
...
provider, the mouse events will be discarded. Add this to your configuration::
...
touch will simulate a multi-touch events. For example::
...
class MouseMotionEventProvider(MotionEventProvider):
    ...
    def update(self, dispatch_fn):
        '''Update the mouse provider (pop events from the queue)'''
        try:
            while True:
