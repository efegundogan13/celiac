...
.. _widget-events-bubbling:

Widget touch events bubbling
---------------------------
...
If a widget has children, the events is passed through its children before
being passed on to the widget after it.

As the :meth:`~kivy.uix.widget.Widget.add_widget` method inserts widgets at
index 0 by default, this means the events goes from the most recently added
...
The label with text "c" gets the events first, "b" second and "a" last. You can
...
would result in the events order "c", "b" then "a" as "c" was actually the last
...
In order to stop this events bubbling, a method can return `True`. This tells
Kivy the events has been handled and the events propagation stops. For example:
...
.. code-block:: python
    ...
    class MyWidget(Widget):
        def on_touch_down(self, touch):
            If <some_condition>:
                # Do stuff here and kill the events
                return True
            else:
                return super(MyWidget, self).on_touch_down(touch)

This approach gives you good control over exactly how events are dispatched
and managed. Sometimes, however, you may wish to let the events be completely
...
.. code-block:: python
    ...
    class MyWidget(Label):
        def on_touch_down(self, touch, after=False):
            if after:
                print "Fired after the events has been dispatched!"
            else:
...
class Widget(WidgetBase):
    ...
    :Events:
        `on_touch_down`: `(touch, )`
            Fired when a new touch events occurs. `touch` is the touch object.
        `on_touch_move`: `(touch, )`
    ...
    .. versionchanged:: 1.0.9
        Everything related to events properties has been moved to the
        :class:`~kivy.events.EventDispatcher`. Event properties can now be used
    ...
    def on_motion(self, etype, me):
        '''Called when a motion events is received.

        :Parameters:
            `etype`: `str`
                Event type, one of "begin", "update" or "end"
            `me`: :class:`~kivy.input.motionevent.MotionEvent`
                Received motion events
        :Returns: `bool`
    ...
    def on_touch_down(self, touch):
        ...
        :Returns: bool
            If True, the dispatching of the touch events will stop.
            If False, the events will continue to be dispatched to the rest
            of the widget tree.
        '''
    ...
    def on_touch_move(self, touch):
        '''Receive a touch move events. The touch is in parent coordinates.
    ...
    def on_touch_up(self, touch):
        '''Receive a touch up events. The touch is in parent coordinates.
    ...
    def register_for_motion_event(self, type_id, widget=None):
        ...
        Override :meth:`on_motion` or bind to `on_motion` events to handle
        the incoming motion events.

        :Parameters:
            `type_id`: `str`
                Motion events type id (eg. "touch", "hover", etc.)
            `widget`: `Widget`
