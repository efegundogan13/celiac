...
:meth:`TextInput.on_text_validate` events)::
...
class TextInput(FocusBehavior, Widget):
    ...
    def on_double_tap(self):
        '''This events is dispatched when a double tap happens
        ...
        events to provide additional functionality.
        '''
        self._select_word()

    def on_triple_tap(self):
        '''This events is dispatched when a triple tap happens
        inside TextInput. The default behavior is to select the
    ...
    def on_quad_touch(self):
        '''This events is dispatched when four fingers are touching
        inside TextInput. The default behavior is to select all text.
        Override this to provide different behavior. Alternatively,
        you can bind to this events to provide additional functionality.
        '''
    ...
    def on_cursor_blink(self, instance, value):
        """trigger blink events reset to switch blinking while focused"""
        self._reset_cursor_blink()

    def _do_blink_cursor(self, dt):
        if not self.cursor_blink:
            # ignore events if not triggered,
    ...
    # cursor position of last IME events
    ...
    '''If True, the :meth:`TextInput.on_text_validate` events will unfocus the
    widget, therefore make it stop listening to the keyboard. When disabled,
    the :meth:`TextInput.on_text_validate` events can be fired multiple times
