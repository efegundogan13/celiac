...
class GestureContainer(EventDispatcher):
    ...
    def complete_stroke(self):
        ...
        are active in the gesture (we only want to dispatch events when
...
class GestureSurface(FloatLayout):
    ...
    :Properties:
        `temporal_window`
            Time to wait from the last touch_up events before attempting
            to recognize the gesture. If you set this to 0, the
            `on_gesture_complete` events is not fired unless the
        ...
        `max_strokes`
            ...
            recognition will start immediately on the final touch_up events.
            If this is set to 0, the `on_gesture_complete` events is not
        ...
        `draw_timeout`
            ...
            `on_gesture_complete` events is fired. If this is set to 0,
    ...
    :Events:
        ...
        `on_gesture_extend` :class:`GestureContainer`
            Fired when a touch_down events occurs within an existing gesture.
        ...
        `on_gesture_complete` :class:`GestureContainer`
            ...
            is reached. Typically you will bind to this events and use
...
# -----------------------------------------------------------------------------
    ...
    def on_touch_move(self, touch):
        ...
        # does not trigger a move events, we would miss it otherwise.
        if self.draw_bbox:
...
# -----------------------------------------------------------------------------
    def _complete_dispatcher(self, dt):
        ...
        the `on_gesture_complete` events for all completed gestures, and remove
        ...
        for idx, g in enumerate(gest):
            ...
            # is complete; need to dispatch _complete or _discard events.
