...
optionally dispatch an on_close events, for instance if a close button
is clicked. This events is used by :class:`Settings` to trigger its own
on_close events.
...
class SettingItem(FloatLayout):
    ...
    :meth:`Settings.on_config_change` events.
...
class InterfaceWithSidebar(BoxLayout):
    ...
    This class also dispatches an events 'on_close', which is triggered
    when the sidebar menu's close button is released. If creating your
    own interface widget, it should also dispatch such an events which
    will automatically be caught by :class:`Settings` and used to
    trigger its own 'on_close' events.
