...
:meth:`App.on_stop` events will be fired.
...
Kivy can be run within an async events loop such as provided by the standard
...
release control of the running context to the asynchronous events loop running
...
:meth:`App.async_run` coroutine must be scheduled to run in the events loop of
...
or :meth:`App.async_run` to run within the given library's async events loop as
in the examples shown below. Kivy is then treated as just another coroutine
that the given library runs in its events loop. Internally, Kivy will use the
...
running within the same async events loop. This is because they are all running
...
coroutines running in the same events loop. Normal single threaded rules apply
to both case.
...
class App(EventDispatcher):
    ...
    :Events:
        ...
        `on_resume`:
            ...
            you have no guarantee that this events will be fired after the
            `on_pause` events has been called.
...
Context.html#getFilesDir()>`_ is returned.
    ...
    async def async_run(self, async_lib=None):
        ...
        scheduled in a running async events loop.
    ...
    def on_start(self):
        '''Event handler for the `on_start` events which is fired after
    ...
    def on_stop(self):
        '''Event handler for the `on_stop` events which is fired when the
    ...
    def on_config_change(self, config, section, key, value):
        ...
        .. versionchanged:: 1.10.1
           Added corresponding ``on_config_change`` events.
        '''
        pass
