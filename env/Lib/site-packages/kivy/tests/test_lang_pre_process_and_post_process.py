...
class TrackCallbacks(object):
    ...
    kv_pre_events = []
    '''Stores values added during the pre events dispatched callbacks.
    '''

    kv_applied_events = []
    '''Stores values added during the applied events dispatched callbacks.
    '''

    kv_post_events = []
    '''Stores values added during the post events dispatched callbacks.
    '''
    ...
    def check_event(widget, event_name, testcase):
        '''Check that the names are added as expected for this events.
        '''
        ...
        for name, value in counter.items():
            testcase.assertEqual(
                value, 1,
                '"{}" was present "{}" times for events "{}" for widget "{} '
                '({})"'.format(name, value, event_name, widget.name, widget))

        testcase.assertEqual(
            set(should_be_in), set(events),
            'Expected and actual events callbacks do not match for events "{}" '
            'for widget "{} ({})"'.format(
