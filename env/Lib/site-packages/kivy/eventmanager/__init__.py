...
list and dispatching `on_motion` events or by using some different logic. It's
also up to manager to dispatch grabbed events if grab feature is supported by
the events (see :meth:`~kivy.input.motionevent.MotionEvent.grab` and
:meth:`~kivy.input.motionevent.MotionEvent.ungrab` methods).

Manager can assign a different dispatch mode to decide how events
...
a local variable or by using events's
...
1. Default dispatch (requires :const:`MODE_DEFAULT_DISPATCH`) - events will go
   through widget's `children` list, starting with the first widget in the
   list until events gets accepted or last widget registered for that events is
   reached. Mode :const:`MODE_DEFAULT_DISPATCH` is assigned by default in
   :class:`~kivy.input.motionevent.MotionEvent` class.
2. Filtered dispatch (requires :const:`MODE_FILTERED_DISPATCH`) - events will go
   only through registered child widgets.
3. No dispatch to children (requires :const:`MODE_DONT_DISPATCH`) - events will
...
'''Assign this mode to make events dispatch through widget's `children` list,
starting with the first widget in the list until events gets accepted or last
widget registered for that events is reached. Widgets after the last registered
widget are ignored.
...
'''Assign this mode to make events dispatch only to child widgets which were
...
'''Assign this mode to prevent events from dispatching to child widgets.
...
class EventManagerBase(object):
    ...
    def dispatch(self, etype, me):
        '''Dispatch events `me` to the widgets in the :attr:`window`.
        ...
        :Returns: `bool`
            `True` to stop events dispatching
