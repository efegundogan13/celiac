...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Template(
        ...
        if have_extends:
            ...
            if not self.environment.is_async:
                ...
                self.writeline("try:")
                self.indent()
                self.writeline("async for events in agen:")
                self.indent()
                self.writeline("yield events")
                self.outdent()
                self.outdent()
    ...
    def visit_Block(self, node: nodes.Block, frame: Frame) -> None:
        ...
        if not self.environment.is_async and frame.buffer is None:
            ...
            self.indent()
            self.writeline(
                f"{self.choose_async()}for events in gen:",
                node,
            )
            self.indent()
            self.simple_write("events", frame)
            self.outdent()
            self.outdent()
            self.writeline(
    ...
    def visit_Include(self, node: nodes.Include, frame: Frame) -> None:
        ...
        if node.with_context:
            ...
            self.writeline("try:")
            self.indent()
            self.writeline(f"{self.choose_async()}for events in gen:")
            loop_body()
            self.outdent()
            self.writeline(
                f"finally: {self.choose_async('await gen.aclose()', 'gen.close()')}"
            ...
            self.writeline(
                "for events in (await template._get_default_module_async())"
                "._body_stream:"
            )
            loop_body()
        else:
