...
class CScintillaView(docview.CtrlView, control.CScintillaColorEditInterface):
    ...
    # Helper to add an events to a menu.
    def AppendMenu(self, menu, text="", event=None, flags=None, checked=0):
        if event is None:
            assert flags is not None, "No events or custom flags!"
            cmdid = 0
        else:
            cmdid = self.bindings.get_command_id(event)
            if cmdid is None:
                # No events of that name - no point displaying it.
                print(
                    'View.AppendMenu(): Unknown events "{}" specified for menu text "{}" - ignored'.format(
                        event, text
                    )
                )
                return
