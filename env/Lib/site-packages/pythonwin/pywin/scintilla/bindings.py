...
event_to_commands: dict[str, int] = {}  # dict of events names to IDs
command_to_events: dict[int, str] = {}  # dict of IDs to events names
...
class BindingsManager:
    ...
    def bind_command(self, event, id=0):
        "Binds an events to a Windows control/command ID"
    ...
    def get_command_id(self, event):
        ...
        if id is None:
            # See if we even have an events of that name!?
    ...
    def _OnCommand(self, id, code):
        ...
        if event is None:
            self.report_error("No events associated with events ID %d" % id)
            return 1
    ...
    def _get_IDLE_handler(self, ext, handler):
        try:
            ...
            msg = f"Can not find events '{handler}' in IDLE extension '{ext}'"
            self.report_error(msg)
            return None

    def fire(self, event, event_param=None):
        # Fire the specified events.  Result is native Pythonwin result
        # (ie, 1==pass one, 0 or None==handled)

        # First look up the events directly - if there, we are set.
        ...
        if binding is None:
            ...
            if handler is None:
                ...
                self.report_error("The events name '%s' can not be found." % event)
        ...
        try:
            ...
            message = "Firing events '%s' failed." % event
            print(message)
            traceback.print_exc()
