    ...
    def __init__(
        ...
        **kw: Any,
        ...
        :param listeners: A list of tuples of the form ``(<eventname>, <fn>)``
            which will be passed to :func:`.events.listen` upon construction.
            This alternate hook to :func:`.events.listen` allows the establishment
            ...
            for use with the :meth:`.DDLEvents.column_reflect` events, however
            note that this events hook may now be associated with the
            :class:`_schema.MetaData` object directly::

                def listen_for_reflect(table, column_info):
                    "handle the column reflection events"
                    # ...
...
class ColumnCollectionMixin:
    ...
    def _check_attach(self, evt: bool = False) -> None:
        ...
        if cols_wo_table:
            # feature #3341 - place events listeners for Column objects
            # such that when all those cols are attached, we autoattach.
            assert not evt, "Should not reach here on events call"
