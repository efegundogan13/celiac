...
class DDLEvents(event.Events[SchemaEventTarget]):
    """
    Define events listeners for schema objects,
    ...
    **Create / Drop Events**
    ...
    The events hooks in this category include :meth:`.DDLEvents.before_create`,
    ...
    .. versionadded:: 2.0 :class:`.DDLEvents` events hooks now take place
    ...
    Example using the :meth:`.DDLEvents.after_create` events, where a custom
    events hook will emit an ``ALTER TABLE`` command on the current connection,
    after ``CREATE TABLE`` is emitted::
        ...
        @events.listens_for(some_table, "after_create")
    ...
    For the :class:`.Index` construct, the events hook will be emitted
    ...
    .. note:: These events hooks are only emitted within the scope of
    ...
    **Reflection Events**
    ...
    The :meth:`.DDLEvents.column_reflect` events is used to intercept
    ...
    **Event Propagation to MetaData Copies**
    ...
    will ensure that a given events handler is propagated to copies of the
    ...
    :meth:`.DDLEvents.after_create` events for both the ``some_table`` and
    ...
    def before_create(
        ...
        :param target: the :class:`.SchemaObject`, such as a
         ...
         object which is the target of the events.
        ...
        :param \**kw: additional keyword arguments relevant
         to the events.  The contents of this dictionary
         may vary across releases, and include the
         list of tables being generated for a metadata-level
         events, the checkfirst flag, and other
         elements used by internal events.

        :func:`.events.listen` accepts the ``propagate=True``
        modifier for this events; when True, the listener function will
        be established for any copies made of the target object,
        i.e. those copies that are generated when
        :meth:`_schema.Table.to_metadata` is used.

        :func:`.events.listen` accepts the ``insert=True``
        modifier for this events; when True, the listener function will
    ...
    def after_create(
        ...
        :func:`.events.listen` also accepts the ``propagate=True``
        modifier for this events; when True, the listener function will
        be established for any copies made of the target object,
        i.e. those copies that are generated when
        :meth:`_schema.Table.to_metadata` is used.
    ...
    def _sa_event_column_added_to_pk_constraint(
        ...
        """internal events hook used for primary key naming convention
        updates.
    ...
    def column_reflect(
        ...
        This events is most easily used by applying it to a specific
        ...
        :class:`_schema.MetaData` that undergo reflection::
            ...
            @events.listens_for(metadata, "column_reflect")
            ...
            # will use the above events hook
        ...
        the board::
            ...
            @events.listens_for(Table, "column_reflect")
            def receive_column_reflect(inspector, table, column_info):
                # receives for all Table objects that are reflected
                ...
        ...
        by :meth:`.reflection.Inspector.get_columns`:
            ...
            * ``default`` - the column's server default value.  This is
              ...
              events can pass a :class:`.FetchedValue`, :class:`.DefaultClause`,
              or :func:`_expression.text` object as well.  Is applied to the
              :paramref:`_schema.Column.server_default` parameter

        The events is called before any action is taken against
