...
class ExecutableDDLElement(roles.DDLRole, Executable, BaseDDLElement):
    ...
    itself an events receiving callable::
    ...
    def against(self, target: SchemaItem) -> Self:
        ...
        is then usable by events handlers and compilation routines in order to
        ...
        an events handler for the :meth:`_events.DDLEvents.before_create` or
        :meth:`_events.DDLEvents.after_create` events, and the events then
    ...
    def execute_if(
        ...
        :class:`_ddl.ExecutableDDLElement` conditionally within an events
        handler.

        Used to provide a wrapper for events listening::
        ...
        :param callable\_: A callable, which will be invoked with
          ...
          arguments:
            ...
            :target:
              ...
              object which is the
              target of this events. May be None if the DDL is executed
              explicitly.
...
class DDL(ExecutableDDLElement):
    ...
    function as DDL events listeners, and can be subscribed to those events
    ...
    Examples::
      ...
      from sqlalchemy import events, DDL

      tbl = Table("users", metadata, Column("uid", Integer))
      events.listen(tbl, "before_create", DDL("DROP TRIGGER users_trigger"))

      spow = DDL("ALTER TABLE %(table)s SET secretpowers TRUE")
      events.listen(tbl, "after_create", spow.execute_if(dialect="somedb"))
