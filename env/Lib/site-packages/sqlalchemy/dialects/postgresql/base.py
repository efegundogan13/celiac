...
the :meth:`.PoolEvents.reset` events hook may be used, as demonstrated
...
will remain consistent with the state of the transaction::
    ...
    @events.listens_for(postgresql_engine, "reset")
...
.. versionchanged:: 2.0.0b3  Added additional state arguments to
   the :meth:`.PoolEvents.reset` events and additionally ensured the events
...
for all connections in a pool using the following events handler, as discussed
at :ref:`schema_set_default_connections`::
    ...
    @events.listens_for(engine, "connect", insert=True)
