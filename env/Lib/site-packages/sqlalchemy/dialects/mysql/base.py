...
In the following example, the events system is used to set the ``sql_mode`` on
the ``first_connect`` and ``connect`` events::
    ...
    @events.listens_for(eng, "connect", insert=True)
...
In the example illustrated above, the "connect" events will invoke the "SET"
