...
the attribute is not available, using events hooks to control the behavior of
BEGIN.
...
* **Enabling modern sqlite3 transaction control via the autocommit connect parameter** (Python 3.12 and above)
  ...
  events hook, however this will only work for sqlite3, as aiosqlite does not yet expose this
  attribute on its ``Connection`` object::

    from sqlalchemy import create_engine, events
...
* **Using SQLAlchemy to emit BEGIN in lieu of SQLite's transaction control** (all Python versions, sqlite3 and aiosqlite)
  ...
  of this command using the :meth:`.PoolEvents.connect` events hook to set the
  ``Connection.isolation_level`` attribute to ``None``::
    ...
    @events.listens_for(engine, "begin")
    def do_begin(conn):
  ...
  as in the example below::
    ...
    @events.listens_for(engine.sync_engine, "begin")
    def do_begin(conn):
...
When using the above mode, any events hooks that set the sqlite3 ``Connection.autocommit``
...
.. seealso::
    ...
    :ref:`event_toplevel` - SQLAlchemy events API.
