...
The public-facing ``Events`` serves as the base class for an events interface;
...
def _is_event_name(name: str) -> bool:
    # _sa_event prefix is special to support internal-only events names.
    # most events names are just plain method names that aren't
    # underscored.
...
class _Dispatch(_DispatchCommon[_ET]):
    """Mirror the events listening definitions of an Events class with
    ...
    the implementation used by the events internals, and allows
...
class Events(_HasEventsDispatch[_ET]):
    """Define events listening functions for a particular target type."""
