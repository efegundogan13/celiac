...
The various listener targets for a particular events class are represented
as attributes, which refer to collections of listeners to be fired off.
These collections can exist at the class level as well as at the instance
level.  An events is fired off using code like this::

    some_object.dispatch.first_connect(arg1, arg2)

Above, ``some_object.dispatch`` would be an instance of ``_Dispatch`` and
``first_connect`` is typically an instance of ``_ListenerCollection``
if events listeners are present, or ``_EmptyListener`` if none are present.
...
class _ClsLevelDispatch(RefCollection[_ET]):
    ...
    def _do_insert_or_append(
        ...
        if not getattr(target, "_sa_propagate_class_events", True):
            raise exc.InvalidRequestError(
                f"Can't assign an events directly to the {target} class"
            )
    ...
    def for_modify(self, obj: _Dispatch[_ET]) -> _ClsLevelDispatch[_ET]:
        """Return an events collection which can be modified.
...
class _CompoundListener(_InstanceLevelDispatch[_ET]):
    ...
    def exec_once(self, *args: Any, **kw: Any) -> None:
        """Execute this events, but only if it has not been
    ...
    def _exec_w_sync_on_first_run(self, *args: Any, **kw: Any) -> None:
        """Execute this events, and use a mutex if it has not been
        executed already for this collection, or was called
