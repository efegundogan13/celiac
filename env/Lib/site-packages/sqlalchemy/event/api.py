...
"""Public API functions for the events system.
...
def _event_key(
    ...
    for evt_cls in _registrars[identifier]:
        ...
        raise exc.InvalidRequestError(
            "No such events '%s' for target '%s'" % (identifier, target)
        )
...
def listen(
    ...
    SQLAlchemy events system, documented at :ref:`event_toplevel`.
    ...
    :param bool insert: The default behavior for events handlers is to append
      the decorated user defined function to an internal list of registered
      events listeners upon discovery. If a user registers a function with
    ...
    :param bool once: Private/Internal API usage. Deprecated.  This parameter
      would provide that an events function would run only once per given
    ...
    :param bool retval: This flag applies only to specific events listeners,
    ...
    .. note::
        ...
        that the target events is being run.   This has implications
        for thread safety, and also means an events cannot be added
...
def listens_for(
    ...
    e.g.::
        ...
        @events.listens_for(UniqueConstraint, "after_parent_attach")
        def unique_constraint_name(const, table):
            const.name = "uq_%s_%s" % (table.name, list(const.columns)[0].name)

    A given function can also be invoked for only the first invocation
    of the events using the ``once`` argument::

        @events.listens_for(Mapper, "before_configure", once=True)
        def on_config():
            do_config()
    ...
    .. seealso::
        ...
        :func:`.listen` - general description of events listening
...
def remove(target: Any, identifier: str, fn: Callable[..., Any]) -> None:
    ...
    :func:`.listen`; all the events registration which proceeded as a result
    ...
    e.g.::
        ...
        @events.listens_for(SomeMappedClass, "before_insert", propagate=True)
        def my_listener_function(*arg):
            pass


        # ... it's removed like this
        events.remove(SomeMappedClass, "before_insert", my_listener_function)
    ...
    .. note::
        ...
        that the target events is being run.   This has implications
        for thread safety, and also means an events cannot be removed
        from inside the listener function for itself.  The list of
        events to be run are present inside of a mutable collection
        that can't be changed during iteration.
