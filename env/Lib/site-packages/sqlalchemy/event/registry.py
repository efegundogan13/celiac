...
By "managed registration", we mean that events listening functions and
...
class EventTarget:
    """represents an events target, that is, something we can listen on
...
def _removed_from_collection(
    ...
    if owner_ref in _collection_to_key:
        ...
        # events key may still be referenced from a clsleveldispatch here
...
class _EventKey(Generic[_ET]):
    ...
    def remove(self) -> None:
        ...
        if key not in _key_to_collection:
            raise exc.InvalidRequestError(
                "No listeners found for events %s / %r / %s "
    ...
    def contains(self) -> bool:
        """Return True if this events key is registered to listen."""
