...
class PoolResetState:
    ...
    the :meth:`.PoolEvents.reset` connection pool events.
    ...
    collection as there is not necessarily an events loop present.

    """

    asyncio_safe: bool
    """Indicates if the reset operation is occurring within a scope where
    an enclosing events loop is expected to be present for asyncio applications.
...
class Pool(log.Identified, event.EventTarget):
    ...
    def __init__(
         ...
         * ``None`` - don't do anything on the connection.
           ...
           :meth:`.PoolEvents.reset` events handler.
         ...
         * ``False`` - same as None, this is here for
         ...
         :meth:`.PoolEvents.reset` events hook may be used which can perform
        ...
        :param events: a list of 2-tuples, each of the form
         ``(callable, target)`` which will be passed to :func:`.events.listen`
         upon construction.   Provided here so that events listeners
...
class ManagesConnection:
    ...
    via the connection pool events hooks, documented at :class:`.PoolEvents`.
...
class ConnectionPoolEntry(ManagesConnection):
    ...
    API code when it is delivered to connection pool events hooks, such as
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    def __connect(self) -> None:
        ...
        try:
            ...
            # in SQLAlchemy 1.4 the first_connect events is not used by
            ...
            # events, so ensure a mutex is used on the first run
