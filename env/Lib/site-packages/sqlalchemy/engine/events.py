...
class ConnectionEvents(event.Events[ConnectionEventsTarget]):
    ...
    The methods here define the name of an events as well as the names of
    members that are passed to listener functions.

    An events listener can be associated with any
    :class:`_engine.Connection` or :class:`_engine.Engine`
    class or instance, such as an :class:`_engine.Engine`, e.g.::

        from sqlalchemy import events, create_engine
        ...
        events.listen(engine, "before_cursor_execute", before_cursor_execute)

    or with a specific :class:`_engine.Connection`::

        with engine.begin() as conn:

            @events.listens_for(conn, "before_cursor_execute")
            def before_cursor_execute(
                conn, cursor, statement, parameters, context, executemany
            ):
                log.info("Received statement: %s", statement)
    ...
    to the database.  The :meth:`.before_cursor_execute` events is
    ...
    as comments, to all executions::
        ...
        @events.listens_for(Engine, "before_cursor_execute", retval=True)
    ...
    .. note:: :class:`_events.ConnectionEvents` can be established on any
       ...
       whether or not its parent :class:`_engine.Engine` has events listeners
       ...
       *not* be available on that :class:`_engine.Connection` instance.
       ...
       instances created subsequent to those events listeners being
       established on the parent :class:`_engine.Engine` class or instance.

    :param retval=False: Applies to the :meth:`.before_execute` and
      :meth:`.before_cursor_execute` events only.  When True, the
      user-defined events function must have a return value, which
        ...
        **kw: Any,
        ...
        if not retval:
            ...
            raise exc.ArgumentError(
                ...
                "events listeners accept the 'retval=True' "
                "argument."
            )
    ...
    def before_execute(
        ...
        This events is good for debugging SQL compilation issues as well
        as early manipulation of the parameters being sent to the database,
        as the parameter lists will be in a consistent format here.

        This events can be optionally established with the ``retval=True``
        flag.  The ``clauseelement``, ``multiparams``, and ``params``
        arguments should be returned as a three-tuple in this case::

            @events.listens_for(Engine, "before_execute", retval=True)
    ...
    def before_cursor_execute(
        ...
        This events is a good choice for logging as well as late modifications
    ...
    def engine_connect(self, conn: Connection) -> None:
        ...
        This events is called typically as the direct result of calling
        ...
        In contrast, this events refers only to the production of a higher level
        :class:`_engine.Connection` wrapper around such a DBAPI connection.

        It also differs from the :meth:`_events.PoolEvents.checkout` events
        ...
        .. seealso::
            ...
            the lower-level pool checkout events
    ...
    def set_connection_execution_options(
        ...
        :meth:`_events.ConnectionEvents.engine_connect` events.
    ...
    def engine_disposed(self, engine: Engine) -> None:
        ...
        This events can be used to indicate that resources related to the
...
class DialectEvents(event.Events[Dialect]):
    """events interface for execution-replacement functions.
    ...
    def _accept_with(
        ...
        if isinstance(target, type):
            ...
            raise exc.InvalidRequestError(
                "The handle_error() events hook as of SQLAlchemy 2.0 is "
    ...
    def handle_error(
        ...
        .. versionchanged:: 2.0 the :meth:`.DialectEvents.handle_error` events
           ...
           :paramref:`_sa.create_engine.pool_pre_ping` parameter. The events
           remains registered by using the :class:`_engine.Engine` as the events
           target, however note that using the :class:`_engine.Connection` as
           an events target for :meth:`.DialectEvents.handle_error` is no longer
           supported.

        This includes all exceptions emitted by the DBAPI as well as
        within SQLAlchemy's statement invocation process, including
        encoding errors and other statement validation errors.  Other areas
        in which the events is invoked include transaction begin and end,
        ...
        events must expect new calling patterns to be present in minor
        releases.

        To support the wide variety of members that correspond to an exception,
        as well as to allow extensibility of the events without backwards
        ...
        * exception re-writing
        ...
        .. versionchanged:: 2.0.0 The :meth:`.DialectEvents.handle_error`
           events hook participates in connection pool "pre-ping" operations.
        ...
        .. versionchanged:: 2.0.5 Added :attr:`.ExceptionContext.is_pre_ping`
           ...
           :meth:`.DialectEvents.handle_error` events hook is triggered within
        ...
        .. versionchanged:: 2.0.5 An issue was repaired that allows for the
           ...
           :meth:`.DialectEvents.handle_error` events hook during
        ...
        which case all further events listeners are bypassed and the
        exception will be raised, after appropriate cleanup as taken
        place::

            @events.listens_for(Engine, "handle_error")
        ...
        .. warning::  Because the
           ...
           events specifically provides for exceptions to be re-thrown as
           the ultimate exception raised by the failed statement,
           **stack traces will be misleading** if the user-defined events
        ...
        Alternatively, a "chained" style of events handling can be
        used, by configuring the handler with the ``retval=True``
        modifier and returning the new exception instance from the
        function.  In this case, events handling will continue onto the
        next handler.   The "chained" exception is available using
        :attr:`.ExceptionContext.chained_exception`::

            @events.listens_for(Engine, "handle_error", retval=True)
            def handle_exception(context):
                if (
    ...
    def do_connect(
        ...
        This events is useful in that it allows the handler to manipulate the
        ...
        dictionary that may also be mutated::
            ...
            @events.listens_for(e, "do_connect")
            def receive_do_connect(dialect, conn_rec, cargs, cparams):
                cparams["password"] = "some_password"

        The events hook may also be used to override the call to ``connect()``
    ...
    def do_executemany(
        ...
        place within the events handler.
    ...
    def do_setinputsizes(
        ...
        This events is emitted in the case where the dialect makes use of the
        ...
        in the ultimate setinputsizes call.   The events may be used to inspect
        ...
        After the events, the ``inputsizes`` dictionary is converted into
