...
class InstrumentationEvents(event.Events[InstrumentationFactory]):
    ...
    to events.listen(), the events will fire off for subclasses
    ...
    def _listen(
        ...
        def listen(target_cls: type, *arg: Any) -> Optional[Any]:
            ...
            # interaction was changed to not rely upon the events system.
...
class InstanceEvents(event.Events[ClassManager[Any]]):
    ...
    e.g.::
        ...
        events.listen(SomeClass, "load", my_load_listener)
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    def _accept_with(
        ...
        if isinstance(target, ClassManager):
            ...
            util.warn_deprecated(
                ...
                "events target, use the 'sqlalchemy.orm.Mapper' class.",
                "2.0",
            )
            return ClassManager
    ...
    def first_init(self, manager: ClassManager[_O], cls: Type[_O]) -> None:
        ...
        This events is called when the ``__init__`` method of a class
        is called the first time for that particular class.    The events
        invokes before ``__init__`` actually proceeds as well as before
        the :meth:`.InstanceEvents.init` events is invoked.
    ...
    def init(self, target: _O, args: Any, kwargs: Any) -> None:
        ...
        events in order to intercept a database load.

        The events is called before the actual ``__init__`` constructor
        of the object is called.  The ``kwargs`` dictionary may be
        modified in-place in order to affect what is passed to
        ``__init__``.

        :param target: the mapped instance.  If
         the events is configured with ``raw=True``, this will
    ...
    def init_failure(self, target: _O, args: Any, kwargs: Any) -> None:
        ...
        The events is invoked after an exception raised by the ``__init__``
        method is caught.  After the events
    ...
    def _sa_event_merge_wo_load(
        ...
        populating dict directly. Therefore the purpose of this events is so
    ...
    def load(self, target: _O, context: QueryContext) -> None:
        ...
        .. warning::
            ...
            During a result-row load, this events is invoked when the
            ...
            if an operation occurs within this events handler that emits
            ...
            the events handler include, but are not necessarily limited to:
            ...
            * accessing attributes on a joined-inheritance subclass that
            ...
            used on the events to prevent this warning; this will ensure that
            the existing loading context is maintained for the object after the
            events is called::

                @events.listens_for(SomeClass, "load", restore_load_context=True)
            ...
            .. versionchanged:: 1.3.14 Added
               ...
               context for an object is restored when the events hook is
               complete; a warning is emitted if the load context of the object
               changes without this flag being set.


        The :meth:`.InstanceEvents.load` events is also available in a
    ...
    def refresh(
        ...
        .. note:: This events is invoked within the loader process before
    ...
    def refresh_flush(
        ...
        This events is the same as :meth:`.InstanceEvents.refresh` except
        ...
        .. note::
            ...
            While the :meth:`.InstanceEvents.refresh_flush` events is triggered
            for an object that was INSERTed as well as for an object that was
            UPDATEd, the events is geared primarily  towards the UPDATE process;
            it is mostly an internal artifact that INSERT actions can also
            trigger this events, and note that **primary key columns for an
            INSERTed row are explicitly omitted** from this events.  In order to
            ...
            if subclass in cls.all_holds:
                ...
                for (
                    ...
                    if propagate or subclass is class_:
                        ...
                        # populate(), we rely upon _EventsHold for all events
...
class MapperEvents(event.Events[mapperlib.Mapper[Any]]):
    ...
    e.g.::
        ...
        events.listen(SomeClass, "before_insert", my_before_insert_listener)
    ...
    * the :class:`_orm.Mapper` class itself indicates listening for all
    ...
    :param raw=False: When True, the "target" argument passed
       ...
       instance's :class:`.InstanceState` management
       object, rather than the mapped instance itself.
    :param retval=False: when True, the user-defined events function
       must have a return value, the purpose of which is either to
       control subsequent events propagation, or to otherwise alter
       the operation in progress by the mapper.   Possible return
       values are:

       * ``sqlalchemy.orm.interfaces.EXT_CONTINUE`` - continue events
         processing normally.
       * ``sqlalchemy.orm.interfaces.EXT_STOP`` - cancel all subsequent
         events handlers in the chain.
    ...
    def instrument_class(self, mapper: Mapper[_O], class_: Type[_O]) -> None:
        ...
        This events is the earliest phase of mapper construction.
        Most attributes of the mapper are not yet initialized.   To
        receive an events within initial mapper construction where basic
        state is available such as the :attr:`_orm.Mapper.attrs` collection,
        the :meth:`_orm.MapperEvents.after_mapper_constructed` events may
        be a better choice.
        ...
        for classes that will be mapped (using the ``propagate=True`` flag)::
            ...
            @events.listens_for(Base, "instrument_class", propagate=True)
    ...
    def after_mapper_constructed(
        ...
        This events is called after the initial constructor for
        :class:`_orm.Mapper` completes.  This occurs after the
        :meth:`_orm.MapperEvents.instrument_class` events and after the
        ...
        This events differs from the
        :meth:`_orm.MapperEvents.before_mapper_configured` events in that it
        ...
        this events is the only one which is appropriate for handlers that
        ...
        .. seealso::
            ...
            events to create new mappers to record change-audit histories on
            objects.
    ...
    def before_mapper_configured(
        ...
        This events is intended to allow a specific mapper to be skipped during
        ...
        :meth:`.MapperEvents.before_mapper_configured` events provides for a
    ...
    def mapper_configured(self, mapper: Mapper[_O], class_: Type[_O]) -> None:
        ...
        The :meth:`.MapperEvents.mapper_configured` events is invoked
        ...
        When the events is called, the mapper should be in its final
        ...
        *will* be fully available, since this style of relationship does not
        ...
        For an events that is guaranteed to have **all** mappers ready
        to go including backrefs that are defined only on other
        mappings, use the :meth:`.MapperEvents.after_configured`
        events; this events invokes only after all known mappings have been
        fully configured.

        The :meth:`.MapperEvents.mapper_configured` events, unlike
        :meth:`.MapperEvents.before_configured` or
        :meth:`.MapperEvents.after_configured`,
        is called for each mapper/class individually, and the mapper is
        passed to the events itself.  It also is called exactly once for
        a particular mapper.  The events is therefore useful for
        ...
        """
        # TODO: need coverage for this events

    def before_configured(self) -> None:
        """Called before a series of mappers have been configured.

        The :meth:`.MapperEvents.before_configured` events is invoked
        ...
        This events can **only** be applied to the :class:`_orm.Mapper` class,
        ...
        for all mappings as a whole::
            ...
            @events.listens_for(Mapper, "before_configured")
            def go(): ...

        Contrast this events to :meth:`.MapperEvents.after_configured`,
        ...
        Theoretically this events is called once per
        application, but is actually called any time new mappers
        are to be affected by a :func:`_orm.configure_mappers`
        call.   If new mappings are constructed after existing ones have
        already been used, this events will likely be called again.  To ensure
        that a particular events is only called once and no further, the
        ``once=True`` argument (new in 0.9.4) can be applied::

            from sqlalchemy.orm import mapper


            @events.listens_for(mapper, "before_configured", once=True)
            def go(): ...
    ...
    def after_configured(self) -> None:
        ...
        The :meth:`.MapperEvents.after_configured` events is invoked
        each time the :func:`_orm.configure_mappers` function is
        ...
        Contrast this events to the :meth:`.MapperEvents.mapper_configured`
        events, which is called on a per-mapper basis while the configuration
        operation proceeds; unlike that events, when this events is invoked,
        ...
        mapped classes.  It is only invoked for all mappings as a whole::
            ...
            @events.listens_for(Mapper, "after_configured")
            def go(): ...
        ...
        ``once=True`` argument (new in 0.9.4) can be applied::
            ...
            @events.listens_for(mapper, "after_configured", once=True)
            def go(): ...
    ...
    def before_insert(
        ...
        .. note:: this events **only** applies to the
        ...
        This events is used to modify local, non-object related
        attributes on the instance before an INSERT occurs, as well
        as to emit additional SQL statements on the given
        connection.

        The events is often called for a batch of objects of the
        same class before their INSERT statements are emitted at
        once in a later step. In the extremely rare case that
        this is not desirable, the :class:`_orm.Mapper` object can be
        configured with ``batch=False``, which will cause
        batches of instances to be broken up into individual
        (and more poorly performing) events->persist->events
        steps.
        ...
        :param target: the mapped instance being persisted.  If
         ...
         instead be the :class:`.InstanceState` state-management
         object associated with the instance.
        :return: No return value is supported by this events.
    ...
    def after_insert(
        ...
        This events is used to modify in-Python-only
    ...
    def before_delete(
        ...
        This events is used to emit additional SQL statements on
        the given connection as well as to perform application
        specific bookkeeping related to a deletion events.
...
class SessionEvents(event.Events[Session]):
    ...
    e.g.::
        ...
        events.listen(Session, "before_commit", my_before_commit)

    The :func:`~.events.listen` function will accept
    ...
    :param raw=False: When True, the "target" argument passed
       to applicable events listener functions that work on individual
    ...
    :param restore_load_context=False: Applies to the
       :meth:`.SessionEvents.loaded_as_persistent` events.  Restores the loader
       context of the object when the events hook is complete, so that ongoing
       ...
       within this events if this flag is not set.
        ...
        if isinstance(target, scoped_session):
            ...
            if not isinstance(target, sessionmaker) and (
                ...
                raise exc.ArgumentError(
                    "Session events listen on a scoped_session "
                    ...
                    **kw: Any,
                    ...
                    if not raw:
                        ...
                        if target is None:
                            ...
                            # garbage collected, no events is emitted
                            return None
                    else:
    ...
    def do_orm_execute(self, orm_execute_state: ORMExecuteState) -> None:
        ...
        This events is invoked for all top-level SQL statements invoked from the
        ...
        will participate in this events.
        This events hook does **not** apply to the queries that are
        emitted internally within the ORM flush process, i.e. the
        process described at :ref:`session_flushing`.

        .. note::  The :meth:`_orm.SessionEvents.do_orm_execute` events hook
        ...
        .. seealso::
            ...
            :meth:`_orm.SessionEvents.do_orm_execute` events which contains
            ...
            of the :meth:`_orm.SessionEvents.do_orm_execute` events hook.

            :ref:`examples_sharding` - the Horizontal Sharding example /
            extension relies upon the
            :meth:`_orm.SessionEvents.do_orm_execute` events hook to invoke a
    ...
    def after_transaction_create(
        ...
        This events differs from :meth:`~.SessionEvents.after_begin`
        in that it occurs for each :class:`.SessionTransaction`
        overall, as opposed to when transactions are begun
        on individual database connections.  It is also invoked
        for nested transactions and subtransactions, and is always
        matched by a corresponding
        :meth:`~.SessionEvents.after_transaction_end` events
        ...
        :param transaction: the target :class:`.SessionTransaction`.
         ...
         is ``None``::
                ...
                @events.listens_for(session, "after_transaction_create")
    ...
    def after_transaction_end(
        ...
        This events differs from :meth:`~.SessionEvents.after_commit`
        in that it corresponds to all :class:`.SessionTransaction`
        objects in use, including those for nested transactions
        and subtransactions, and is always matched by a corresponding
        :meth:`~.SessionEvents.after_transaction_create` events.
    ...
    def after_commit(self, session: Session) -> None:
        ...
        .. note::
            ...
            when the :meth:`~.SessionEvents.after_commit` events is invoked,
    ...
    def after_rollback(self, session: Session) -> None:
        ...
        Note that this events only fires when the *actual* rollback against
        the database occurs - it does *not* fire each time the
        :meth:`.Session.rollback` method is called, if the underlying
        DBAPI transaction has already been rolled back.  In many
        cases, the :class:`.Session` will not be in
        an "active" state during this events, as the current
        transaction is not valid.   To acquire a :class:`.Session`
        which is active after the outermost rollback has proceeded,
        use the :meth:`.SessionEvents.after_soft_rollback` events, checking the
    ...
    def after_soft_rollback(
        ...
        by first checking the :attr:`.Session.is_active` flag::
            ...
            @events.listens_for(Session, "after_soft_rollback")
    ...
    def after_flush(
        ...
        .. warning:: This events runs after the :class:`.Session` has emitted
           ...
           emitted within this events such as loads of related items
    ...
    def after_attach(self, session: Session, instance: _O) -> None:
        ...
        .. note::
           ...
           As of 0.8, this events fires off *after* the item
           has been fully associated with the session, which is
           different than previous releases.  For events
           handlers that require the object not yet
           be part of session state (such as handlers which
           may autoflush while the target object is not
           yet complete) consider the
           new :meth:`.before_attach` events.
    ...
    def after_bulk_update(self, update_context: _O) -> None:
        ...
        .. legacy:: The :meth:`_orm.SessionEvents.after_bulk_update` method
           is a legacy events hook as of SQLAlchemy 2.0.   The events
    ...
    def transient_to_pending(self, session: Session, instance: _O) -> None:
        ...
        This events is a specialization of the
        :meth:`.SessionEvents.after_attach` events which is only invoked
    ...
    def pending_to_persistent(self, session: Session, instance: _O) -> None:
        ...
        This events is invoked within the flush process, and is
        similar to scanning the :attr:`.Session.new` collection within
        the :meth:`.SessionEvents.after_flush` events.  However, in this
        case the object has already been moved to the persistent state
        when the events is called.
    ...
    def detached_to_persistent(self, session: Session, instance: _O) -> None:
        ...
        .. note::
            ...
            deleted when this events is called.  To detect deleted objects,
            check the ``deleted`` flag sent to the
            :meth:`.SessionEvents.persistent_to_detached` to events after the
            flush proceeds, or check the :attr:`.Session.deleted` collection
            within the :meth:`.SessionEvents.before_flush` events if deleted
    ...
    def loaded_as_persistent(self, session: Session, instance: _O) -> None:
        ...
        This events is invoked within the ORM loading process, and is invoked
        very similarly to the :meth:`.InstanceEvents.load` events.  However,
        the events here is linkable to a :class:`.Session` class or instance,
    ...
    def persistent_to_deleted(self, session: Session, instance: _O) -> None:
        ...
        This events is invoked when a persistent object's identity
        ...
        :meth:`.SessionEvents.deleted_to_persistent` events is called.
        ...
        the :meth:`.SessionEvents.persistent_to_deleted` events is therefore
    ...
    def deleted_to_persistent(self, session: Session, instance: _O) -> None:
        ...
        :meth:`.Session.rollback`.   The events is not called under
    ...
    def deleted_to_detached(self, session: Session, instance: _O) -> None:
        ...
        This events is invoked when a deleted object is evicted
    ...
    def persistent_to_detached(self, session: Session, instance: _O) -> None:
        ...
        This events is invoked when a persistent object is evicted
...
class AttributeEvents(event.Events[QueryableAttribute[Any]]):
    ...
    :meth:`_orm.AttributeEvents.append` events::

        from sqlalchemy import events


        @events.listens_for(MyClass.collection, "append", propagate=True)
        def my_append_listener(target, value, initiator):
            print("received append events for target: %s" % target)
    ...
    :func:`.events.listen` or :func:`.events.listens_for`, such as below,
    illustrated using the :meth:`_orm.AttributeEvents.set` events::
    ...
    as when using mapper inheritance patterns::
        ...
        @events.listens_for(MySuperClass.attr, "set", propagate=True)
        def receive_set(target, value, initiator):
            print("value set: %s" % target)

    The full list of modifiers available to the :func:`.events.listen`
    and :func:`.events.listens_for` functions are below.

    :param active_history=False: When True, indicates that the
      "set" events would like to receive the "old" value being
    ...
    :param raw=False: When True, the "target" argument to the
      events will be the :class:`.InstanceState` management
      object, rather than the mapped instance itself.
    :param retval=False: when True, the user-defined events
    ...
    def append(
        ...
        *,
        ...
        """Receive a collection append events.

        The append events is invoked for each element as it is appended
        to the collection.  This occurs for single-item appends as well
        as for a "bulk replace" operation.

        :param target: the object instance receiving the events.
        ...
        :param initiator: An instance of :class:`.attributes.Event`
          representing the initiation of the events.  May be modified
          from its original value by backref handlers in order to control
          chained events propagation, as well as be inspected for information
          about the source of the events.
        :param key: When the events is established using the
         ...
         to the events at all if the the
         :paramref:`.AttributeEvents.include_key`
         was not used to set up the events; this is to allow backwards
         compatibility with existing events handlers that don't include the
         ``key`` parameter.

         .. versionadded:: 2.0

        :return: if the events was registered with ``retval=True``,
    ...
    def append_wo_mutation(
        ...
        *,
        ...
        """Receive a collection append events where the collection was not
        actually mutated.

        This events differs from :meth:`_orm.AttributeEvents.append` in that
        ...
        The events does not have a return value and the identity of the
        given object cannot be changed.

        The events is used for cascading objects into a :class:`_orm.Session`
        when the collection has already been mutated via a backref events.
        ...
        :return: No return value is defined for this events.
    ...
    def bulk_replace(
        ...
        *,
        ...
        """Receive a collection 'bulk replace' events.

        This events is invoked for a sequence of values as they are incoming
        ...
        :meth:`.AttributeEvents.append` events.    When using both of these
        events, note that a bulk replace operation will invoke
        the :meth:`.AttributeEvents.append` events for all new items,
        even after :meth:`.AttributeEvents.bulk_replace` has been invoked
        for the collection as a whole.  In order to determine if an
        :meth:`.AttributeEvents.append` events is part of a bulk replace,
        ...
        incoming initiator::
            ...
            @events.listens_for(SomeObject.collection, "bulk_replace")
            def process_collection(target, values, initiator):
                values[:] = [_make_value(value) for value in values]


            @events.listens_for(SomeObject.collection, "append", retval=True)
        ...
        :param initiator: An instance of :class:`.attributes.Event`
          representing the initiation of the events.
        :param keys: When the events is established using the
         :paramref:`.AttributeEvents.include_key` parameter set to
    ...
    def remove(
        ...
        *,
        ...
        """Receive a collection remove events.
    ...
    def init_scalar(
        ...
        r"""Receive a scalar "init" events.

        This events is invoked when an uninitialized, unpersisted scalar
        ...
        events here can be used to customize what value is actually returned,
        with the assumption that the events listener would be mirroring
        ...
        events handler can also be used to **set** the newly returned value, so
        ...
        * By establishing the ``retval=True`` flag, the value we return
          ...
          Without this flag, the events is assumed to be a passive observer
        ...
        * The ``propagate=True`` flag is significant if the mapped class
          ...
          events listener.  Without this flag, an inheriting subclass will
          not use our events handler.

        In the above example, the attribute set events
        ...
        :param target: the object instance receiving the events.
         If the listener is registered with ``raw=True``, this will
         be the :class:`.InstanceState` object.
        :param value: the value that is to be returned before this events
         listener were invoked.  This value begins as the value ``None``,
         however will be the return value of the previous events handler
    ...
    def dispose_collection(
        ...
        """Receive a 'collection dispose' events.
    ...
    def modified(self, target: _O, initiator: Event) -> None:
        """Receive a 'modified' events.

        This events is triggered when the :func:`.attributes.flag_modified`
        function is used to trigger a modify events on an attribute without
...
class QueryEvents(event.Events[Query[Any]]):
    ...
    .. legacy:: The :class:`_orm.QueryEvents` events methods are legacy
    ...
    :meth:`_orm.SessionEvents.do_orm_execute` events hook.
    ...
    def before_compile(self, query: Query[Any]) -> None:
        ...
        .. deprecated:: 1.4  The :meth:`_orm.QueryEvents.before_compile` events
           is superseded by the much more capable
           :meth:`_orm.SessionEvents.do_orm_execute` hook.   In version 1.4,
           the :meth:`_orm.QueryEvents.before_compile` events is **no longer
        ...
        This events is intended to allow changes to the query given::

            @events.listens_for(Query, "before_compile", retval=True)
        ...
        The events should normally be listened with the ``retval=True``
        parameter set, so that the modified query may be returned.

        The :meth:`.QueryEvents.before_compile` events by default
        will disallow "baked" queries from caching a query, if the events
        ...
        re-establish the query being cached, apply the events adding the
        ``bake_ok`` flag::

            @events.listens_for(Query, "before_compile", retval=True, bake_ok=True)
            def my_event(query):
                for desc in query.column_descriptions:
                    if desc["type"] is User:
                        entity = desc["entity"]
        ...
        When ``bake_ok`` is set to True, the events hook will only be invoked
        ...
        .. versionadded:: 1.3.11  - added the "bake_ok" flag to the
           :meth:`.QueryEvents.before_compile` events and disallowed caching via
           the "baked" extension from occurring for events handlers that
    ...
    def before_compile_update(
        ...
        .. deprecated:: 1.4  The :meth:`_orm.QueryEvents.before_compile_update`
           events is superseded by the much more capable
           :meth:`_orm.SessionEvents.do_orm_execute` hook.

        Like the :meth:`.QueryEvents.before_compile` events, if the events
        ...
        :class:`_query.Query` object returned, as in ::
            ...
            @events.listens_for(Query, "before_compile_update", retval=True)
    ...
    def before_compile_delete(
        ...
        Like the :meth:`.QueryEvents.before_compile` events, this events
        should be configured with ``retval=True``, and the modified
        :class:`_query.Query` object returned, as in ::

            @events.listens_for(Query, "before_compile_delete", retval=True)
