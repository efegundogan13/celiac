...
class ORMExecuteState(util.MemoizedSlots):
    ...
    to the :meth:`.SessionEvents.do_orm_execute` events hook.
    ...
    def invoke_statement(
        ...
        statement for which the :meth:`.SessionEvents.do_orm_execute` events is
        being currently invoked.    The use case for this is for events handlers
...
class SessionTransaction(_StateChange, TransactionalContext):
    ...
    enumeration indicating the source events that led to constructing
                    ...
                    if join_transaction_mode == "conditional_savepoint":
                        ...
                        if local_connect:
                            util.warn(
                                ...
                                "This is usually caused by a core events, "
    ...
    def close(self, invalidate: bool = False) -> None:
        ...
        # events hook below, and in 2.0 I changed it this way for some reason,
...
class Session(_SessionClassMethods, EventTarget):
    ...
    def commit(self) -> None:
        ...
        detected, but will still invoke events handlers and object expiration
        rules.
        ...
        *,
        ...
        if events_todo:
            if compile_state_cls is not None:
                # for events handlers, do the orm_pre_session_exec
                # pass ahead of the events handlers, so that things like
        ...
        if compile_state_cls is not None:
            ...
            # events hooks, this will re-run the steps that interpret
            # new execution_options into load_options / update_delete_options,
            # which we assume the events hook might have updated.
            ...
            if obj is not None:
                ...
                if (
                    ...
                    raise exc.FlushError(
                        ...
                        "time, such as within a load() events."
                        % state_str(state)
                    )
                ...
                if (
                    ...
                    util.warn(
                        ...
                        "load operations occurring inside of an events handler "
    ...
    def enable_relationship_loading(self, obj: object) -> None:
        ...
        **not** participate in any persistence operations; its state
        ...
        may not fire off a backref events, if the effective value
    ...
    def _flush_warning(self, method: Any) -> None:
        util.warn(
            ...
            "events listeners or connection-level operations instead." % method
        )
        ...
        try:
            ...
            if not objects and self.identity_map._modified:
                ...
                util.warn(
                    ...
                    "within inner-flush events handlers have been "
                    "reset, and will not result in database updates. "
                    "Consider using set_committed_value() within "
                    "inner-flush events handlers to avoid this warning." % len_
                )
    ...
    def is_modified(
        ...
        * Scalar attributes may not have recorded the previously set
          ...
          most cases does not need the "old" value when a set events occurs, so
