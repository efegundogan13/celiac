...
class QueryableAttribute(
    ...
    def __init__(
        ...
        if manager:
            # propagate existing events listeners from
...
class AttributeEventToken:
    ...
    Serves as an indicator of the source of the events and also provides
    ...
    events handlers, and is used to control the propagation of operations
    across two mutually-dependent attributes.

    .. versionchanged:: 2.0  Changed the name from ``AttributeEvent``
       to ``AttributeEventToken``.

    :attribute impl: The :class:`.AttributeImpl` which is the current events
     initiator.
...
class CollectionAttributeImpl(HasCollectionAdapter, AttributeImpl):
    ...
    def fire_pre_remove_event(
        ...
        """A special events used for pop() operations.

        The "remove" events needs to have the item to be removed passed to
        it, which in the case of pop from a set, we don't have a way to access
        the item before the operation.   the events is used for all pop()
    ...
    def append(
        ...
        if collection is PASSIVE_NO_RESULT:
            ...
            ), "Collection was loaded during events handling."
    ...
    def set_committed_value(
        ...
        if self.key in state._pending_mutations:
            # pending items exist.  issue a modified events,
...
def backref_listeners(
    ...
    # that is specific to how we might arrive at each events.  a marker
    ...
    def _acceptable_key_err(child_state, initiator, child_impl):
        raise ValueError(
            ...
            'triggers a modify events on attribute "%s" '
            'via the backref "%s".'
            % (
        ...
        if (
            ...
            if (
                ...
                if not check_for_dupes_on_remove or not util.has_dupes(
                    # when this events is called, the item is usually
    ...
    # TODO: need coverage in test/orm/ of remove events
    event.listen(
        attribute,
        "remove",
...
def set_attribute(
    ...
    :param initiator: an instance of :class:`.Event` that would have
     been propagated from a previous events listener.  This argument
     is used when the :func:`.set_attribute` function is being used within
     an existing events listening function where an :class:`.Event` object
...
def flag_modified(instance: object, key: str) -> None:
    ...
    establishes an unconditional change events for the given attribute.
