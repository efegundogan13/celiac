...
def __get_disp_and_event_classes(dispatch):
    ...
    # the events sink class and the user class.
...
def WithEvents(disp, user_event_class):
    ...
    single object that is both the interface and the events handler.  Note that
    the events handler instance will *not* be able to use 'self.' to refer to
...
def getevents(clsid):
    ...
    the events source. Call the 'close' method to break the chain.
