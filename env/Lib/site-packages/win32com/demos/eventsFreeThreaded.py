...
# sychronization can be used.  In this sample we just wait on win32 events
# objects.
...
# * The first OnDocumentComplete events is not a reliable indicator that the
...
class ExplorerEvents:
    def __init__(self):
        # We reuse this events for all events.
    ...
    def OnDocumentComplete(self, pDisp=pythoncom.Empty, URL=pythoncom.Empty):
        ...
        print("OnDocumentComplete events processed on thread %d" % thread)
        # Set the events our main thread is waiting on.
        win32event.SetEvent(self.event)

    def OnQuit(self):
        thread = win32api.GetCurrentThreadId()
        print("OnQuit events processed on thread %d" % thread)
        win32event.SetEvent(self.event)
...
def TestExplorerEvents():
    ...
    rc = win32event.WaitForSingleObject(iexplore.event, 2000)
    if rc != win32event.WAIT_OBJECT_0:
        print("OnQuit events FAILED to fire!!!")

    iexplore = None
    print("Finished the IE events sample!")
